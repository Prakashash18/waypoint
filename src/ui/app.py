...
from src.agent.api_tracker import tracker
from src.agent.flight_status import FlightStatusService
...
def serve_spa(path):
    ...
    There is one UI now. It lives at /app, because its assets are built with
    that base path, so the root simply redirects there.
    """
